...
function processEvent(event) {
	...
	eventRecord = {id: event["id"], element: eventElement, object: event};
...
function processEventDetails(eventElement, eventDetails) {
	...
	panelBodyParent = panelBody.parentElement;
	panelBody.parentElement.removeChild(panelBody);
	
	eventBody = renderEventBody(eventDetails);
	panelBodyParent.appendChild(eventBody);
	
	// Add Host label
	hostPic = eventElement.children[1].lastChild;
	hostLabel = document.createElement("DIV");
	hostLabel.id = "host_label";
	hostLabel.innerHTML = eventDetails["owner_name"];
	eventElement.children[1].insertBefore(hostLabel, hostPic);
...
function renderEventBody(event) {
	...
	eventBody.className = "panel-body event_body";
	eventBody.id = "event_body_" + event["id"];
	eventBody.innerHTML = "<div class='panel_body_text'>" + event["description"] + "</div>";
	...
	if (event["status"] != "owner") {
		...
		circleDiv.style.background = "url(" + event["owner_pic"] + ") no-repeat";
		circleDiv.style.backgroundSize = "100% auto";
		eventBody.appendChild(circleDiv);
	}
	
	
	// Add action buttons
	detailButtonGroup = getDetailButtonGroup();
...
function eventClick(e) {
	...
	invites_list = document.getElementById("invites_list");
	invites_list.parentNode.removeChild(invites_list);
	
	host_label = document.getElementById("host_label");
	host_label.parentNode.removeChild(host_label);
	
	detail_btn_group = document.getElementById("detail_btn_group");
	detail_btn_group.parentNode.removeChild(detail_btn_group);
