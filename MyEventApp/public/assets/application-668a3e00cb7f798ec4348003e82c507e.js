...
window.onload = function(){ 
	document.getElementById('event_description').onkeyup = check_tag($("#event_description").val());
	document.getElementById('tags').onkeyup = check_tag($("#tags").val());
}
...
function check_tag(value){
	...
	if(value[value.length-1] == " " ){
		...
		if(words.length > 0){
			var final_word = words[words.length-2];
			if(tags.indexOf(final_word) != -1)
				return;
			var xhr        = new XMLHttpRequest();
			xhr.onreadystatechange = xhr_handler;
			var url = 'get_tag?word=' + encodeURIComponent(final_word);
			xhr.open("GET", url);
			xhr.send();			}
	}
}
