...
function processEventDetails(eventDetails) {
	...
	for (var invite in invitesArray) {
		...
		if (inviteCopy["accepted"] == true) {
			wrapper.className = "well well-sm profile_well_success";
		} else if (inviteCopy["seen"] == true) {
			wrapper.className = "well well-sm profile_well_danger";
		} else {
			wrapper.className = "well well-sm profile_well_warning";
		}
		...
		lastViewed.children[1].appendChild(wrapper);
	}
}
