...
function calClick(e) {
	...
	var $elem = $(item);
	$elem.animate({top:(0 + window.pageYOffset) + "px", left:"0px", maxWidth:"100%", width:"100%", height:"118%"});
...
function eventClick(e) {
	...
	var $elem = $(item);
	$elem.animate({top:(item.getBoundingClientRect().top + window.pageYOffset) + "px",
			left:item.getBoundingClientRect().left + "0px",
			maxWidth:"82%",
			width:"auto",
			height:"70px"});
	
	var $header = $(item.firstElementChild);
	$header.animate({fontSize:"14px"});
	
	var $description = $(item.children[1]);
	$description.animate({fontSize:"14px"});
