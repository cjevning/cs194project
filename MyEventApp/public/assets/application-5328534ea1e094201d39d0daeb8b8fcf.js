	...
	function acceptXhrHandler() {
		...
		if (this.status != 200) {
			...
			if (acceptXhr.responseText != "-1") {
				updateLocalEventModel(acceptXhr.responseText, "accept", id);
			}
		}
	}
}
...
function sendEventRejection(id) {
	...
	function rejectXhrHandler() {
		...
		if (this.status != 200) {
			...
			if (message != "-1") {
				updateLocalEventModel(message, "reject", id);
			}
...
function sendEventMaybe(id) {
	...
	function maybeXhrHandler() {
		...
		if (this.status != 200) {
			...
			if (message != "-1") {
				updateLocalEventModel(message, "maybe", id);
			}
			if (reply["replacementEvent"] != null) {
				processEvent(reply["replacementEvent"]);
			}
...
function updateLocalEventModel(id, updateType, oldID) {
	eventRecord = findEventRecordById(id);
	if (updateType == "accept") {
		acceptedRecord = findEventRecordById(oldID);
		invitations = acceptedRecord.object["invitations"];
		for (i = 0; i < invitations.length; i++) {
			if (invitations[i]["self"] == true) {
				acceptedRecord["accepted"] = true;
				acceptedRecord["rejected"] = false;
				acceptedRecord["maybe"] = false;
			}
		}
		...
		maybeRecord = findEventRecordById(oldID);
		invitations = maybeRecord.object["invitations"];
		for (i = 0; i < invitations.length; i++) {
			if (invitations[i]["self"] == true) {
				maybeRecord["maybe"] = true;
				maybeRecord["accepted"] = false;
				maybeRecord["rejected"] = false;
			}
		}
		displayAddMaybe(eventRecord);
	}
