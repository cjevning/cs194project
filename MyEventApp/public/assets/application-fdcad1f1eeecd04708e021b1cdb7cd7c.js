...
function newEvent() {
	$.ajax({
		...
		success:function(result){
			...
			newOverlay.setAttribute("id","new_overlay");
			newOverlay.style.top = $(window).height() + "px";
			newOverlay.innerHTML = obj["content"];
			document.body.appendChild(newOverlay);
			var $newOver = $(newOverlay);
			$newOver.animate({top:(window.pageYOffset + 56) + "px"}, "slow", "swing", function() {loadFriends(obj["data"])});
			
			mainButton = document.getElementById("main_button");
			mainButton.className = "glyphicon glyphicon-remove";
...
function createFriends(json_friends) {
