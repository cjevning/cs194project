...
function sendEventRequest() {
	...
	function eventXhrHandler() {
		...
		if (this.status != 200) {
			...
			JSONObject obj = new JSONObject(eventXhr.responseText);
			JSONArray eventArray = obj.getJSONArray("eventList");
			console.log(eventArray);
			for (var i = 0; i < eventArray.length; i++) {
				JSONObject event = (JSONObject)eventArray.get(i);
				processEvent(event);
			}
		}
	}
...
function createEventElement(event) {
	...
	heading.innerHTML = event.getString("name");
	panelBody = document.createElement("DIV");
	panelBody.className = "panel-body";
	panelBody.innerHTML = event.getString("description");
