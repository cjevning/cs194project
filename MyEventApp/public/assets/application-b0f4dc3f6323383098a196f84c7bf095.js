	...
	if (updateType == "accept") {
		...
		for (i = 0; i < invitations.length; i++) {
			if (invitations[i]["self"] == true) {
				eventRecord.object["accepted"] = true;
				eventRecord.object["rejected"] = false;
				eventRecord.object["maybe"] = false;
			}
		}
		...
		for (i = 0; i < invitations.length; i++) {
			if (invitations[i]["self"] == true) {
				eventRecord.object["maybe"] = true;
				eventRecord.object["accepted"] = false;
				eventRecord.object["rejected"] = false;
			}
		}
		displayAddMaybe(eventRecord);
